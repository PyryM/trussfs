...
fn event_to_string(evt: notify::Event) -> String {
  let paths: Vec<String> = evt.paths.into_iter().map(|p| p.to_string_lossy().into_owned()).collect();
...
impl FileWatcher {
  pub fn new() -> Result<Self, String> {
    ...
    let watcher = RecommendedWatcher::new(move |res: Result<Event, Error>| {
        match res {
          Ok(evt) => {
  ...
  pub fn watch(&mut self, path: String, recursive: bool) -> Result<(), String> {
    ...
    self.inner.watch(path.as_ref(), mode).map_err(|e| e.to_string())
  }

  pub fn poll_events(&mut self) -> StringList {
    let mut events = StringList::new();
    while let Ok(evt) = self.recv.try_recv() {
      events.push(CString::new(evt).expect("It should be impossible for a std::String to contain null bytes!"));
    }
    events
  }
}